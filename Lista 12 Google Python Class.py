...
def not_bad(s):
  tem_bad,tem_not = s.find("bad"),s.find("not")
  if tem_bad != -1 and tem_not != -1:
    if s.index("bad") > s.index("not"):
      return s.replace("not that bad","good").replace("not so bad","good")
  return s
...
def inip2(n):
  c = 0
  potencia = ''
  while potencia != str(n):
    potencia = str(2 ** c)[:len(str(n))]
    c += 1
  return c-1
